<?php
class Tx_ArdMcm_Lib_PidHandler {
	protected $rootPid=0;
	...
	function getPid($path,$rootpid=0,$firstcall=true) {
		...
		if (!$path && $firstcall) throw new Exception('Tx_ArdMcm_Lib_PidHandler getPid : Path is not set!');
		...
		if (!$rootpid) {
			...
			if (!$rootpid) throw new Exception('Tx_ArdMcm_Lib_PidHandler : Root pid for path getPid: '.$path.' is not set!');
		}
		...
		if (count($pathArray) && $path) {
			...
			if ($res===false){
				$err=mysql_error();
				throw new Tx_ArdMcm_Exception_SqlException('getPid :  Sql Error '.$GLOBALS['TYPO3_DB']->SELECTquery($fields,$from, $where,'',$orderby).'--'.$err,404);
			}
			...
			if (!$cnt)  {
				...
				if ($res===false){
					$err=mysql_error();
					throw new Tx_ArdMcm_Exception_SqlException('Tx_ArdMcm_Lib_PidHandler->getPid :  Sql Error '.$GLOBALS['TYPO3_DB']->INSERTquery($from, $data).'--'.$err,404);
				}
	...
	function getPath($uid) {
		...
		if (!$cnt)  {
			...
			while($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($res)) {
				...
				$p=$this->getPath($row['pid']);
				return  ($p?$p.'/':'').$row['title'];
			}
			// We should never come here
			return $path;
		}	
	...
	function getGroupUid($path,$rootpid=0,$firstcall=true) {
		...
		if (!$path && $firstcall) throw new Exception('Tx_ArdMcm_Lib_PidHandler getGroupUid : Path is not set!');
		$pathArray=explode('/',$path);
		...
		if ($res===false){
			$err=mysql_error();
			throw new Tx_ArdMcm_Exception_SqlException('getGroupUid :  Sql Error '.$GLOBALS['TYPO3_DB']->SELECTquery($fields,$from, $where,'',$orderby).'--'.$err,404);
		}
		...
		if (!$cnt)  {
				...
				if ($res===false){
					$err=mysql_error();
					throw new Tx_ArdMcm_Exception_SqlException('Tx_ArdMcm_Lib_PidHandler->getGroupUid :  Sql Error '.$GLOBALS['TYPO3_DB']->INSERTquery($from, $data).'--'.$err,404);
				}
				$uid=$GLOBALS['TYPO3_DB']->sql_insert_id();
				return $uid;
		} else {
