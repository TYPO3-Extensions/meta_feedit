...
class tx_metafeedit_export {
	...
	function getCSV(&$content,&$caller,$exportFile='') {
		//we remove execution time limit;
		set_time_limit(0);
	...
	function addFonts() {
		...
		$this->addFont('georgia','I','georgiai.php');
		$this->addFont('georgia','B','georgiab.php');
		$this->addFont('georgia','BI','georgiabi.php');
	...
	*/
	...
	/**
	 * Generates Excel file from xml data
	 * XML Data is generated by meta_feedit in the form :
	 * <table>
	 * <tr><th></th>...</tr>
	 * <tr><td></td>...</tr>
	 * ...
	 * </table>
	 * @param string $content
	 * @param tx_metafeedit_user_feAdmin $caller
	 * @param string $exportFile : name of file to export to if called from CLI
	 * @return void (we die after generating file)
	 * 
	 * @todo  perfrormance improvements :
	 * - $content is xml data, the bigger it is the more memory it requires ... Maybe write to file instead of memory.
	 * - XMl data has to be read entirely in memory  ? other solution ? XMLReader allows to create XML File cursors ...
	 * - can Excel data be streamed to disk ?
	 */
	function getEXCEL(&$content,tx_metafeedit_user_feAdmin &$caller,$exportFile='') {
		//we remove execution time limit;
		set_time_limit(0);
