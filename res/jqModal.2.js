...
// *AND* ...
H[s]={c:$$$.extend(o,p),a:false,w:jQuery(this).addClass('jqmID'+s),s:s};
...
open:function(s,t){var h=H[s],c=h.c,cc='.'+c.closeClass,z=/^\d+$$$/.test(h.w.css('z-index'))&&h.w.css('z-index')||c.zIndex,o=jQuery('<div></div>').css({height:'100%',width:'100%',position:'fixed',left:0,top:0,'z-index':z-1,opacity:c.overlay/100});h.t=t;h.a=true;h.w.css('z-index',z);
 ...
 if(ie6&&jQuery('html,body').css({height:'100%',width:'100%'})&&o){o=o.css({position:'absolute'})[0];for(var y in {Top:1,Left:1})o.style.setExpression(y.toLowerCase(),"(_=(document.documentElement.scroll"+y+" || document.body.scroll"+y+"))+'px'");}
 ...
 if(c.ajax) {var r=c.target||h.w,u=c.ajax,r=(typeof r == 'string')?jQuery(r,h.w):jQuery(r),u=(u.substr(0,1) == '@')?jQuery(t).attr(u.substring(1)):u;
  ...
  r.load(u,function(){c.onLoad&&c.onLoad.call(this,h);cc&&h.w.jqmAddClose(jQuery(cc,h.w));O(h);});}
 
 // ELSE the modal content is NOT to be loaded via ajax;
 //  Attach closing events to elements inside the modal that match the closingClass
 else cc&&h.w.jqmAddClose(jQuery(cc,h.w));
...
close:function(s){var h=H[s];h.a=false;
 ...
 h.c.toTop&&h.o&&jQuery('#jqmP'+h.w[0]._jqm).after(h.w).remove();
...
alert(2);
var s=0,H=$$$.jqm.hash,A=[],ie6=$$$.browser.msie&&($$$.browser.version == "6.0"),i=jQuery('<iframe src="javascript:false;document.write(\'\');" class="jqm"></iframe>').css({opacity:0}),
...
O=function(h){if(ie6)h.o&&h.o.html('<p style="width:100%;height:100%"/>').prepend(i)||(!jQuery('iframe.jqm',h.w)[0]&&h.w.prepend(i)); f(h);},
...
F=function(t){jQuery()[t]("keypress",x)[t]("keydown",x)[t]("mousedown",x);},
...
x=function(e){var h=H[A[A.length-1]],r=(!jQuery(e.target).parents('.jqmID'+h.s)[0]);r&&f(h);return !r;},
...
//  or else expand the expando with the current serial array
 jQuery(e).each(function(){if(this[y])$$$.extend(this[y],s);
 ...
 else{this[y]=s;jQuery(this).click(function(){for(var i in {jqmShow:1,jqmHide:1})for(var s in this[i])if(H[this[i][s]])H[this[i][s]].w[i](this);return false;});}});return w;};
})(jQuery);