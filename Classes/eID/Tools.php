 ...
 *
 * $Id: class.tslib_eidtools.php 7905 2010-06-13 14:42:33Z ohader $
 *
 ...
 * @subpackage tslib
 */
final class Tx_MetaFeedit_EID_Tools {
	...
	public static function initFeUser()	{
		...
		self::connectDB();
		$TTA[]= "<br/>eID eidtools after connectdb  Elapsed Time :".(microtime(true)-$GLOBALS['g_TT']).' s';
		// Get TSFE instance. It knows how to initialize the user. We also
		// need TCA because services may need extra tables!
		self::initTCA();
		$TTA[]= "<br/>eID eidtools after initTCA  Elapsed Time :".(microtime(true)-$GLOBALS['g_TT']).' s';
		$tsfe = self::getTSFE();
		/* @var $tsfe tslib_fe */

		$tsfe->initFEuser();
		$TTA[]= "<br/>eID eidtools after initFEUser Elapsed Time :".(microtime(true)-$GLOBALS['g_TT']).' s';
	...
	public static function connectDB()	{
		...
		if (!$dbConnected) {
			...
			$TTA[]= "<br/>eID eidtools before pconnect Elapsed Time :".(microtime(true)-$GLOBALS['g_TT']).' s';	
			if ($GLOBALS['TYPO3_DB']->sql_pconnect(TYPO3_db_host, TYPO3_db_username, TYPO3_db_password) &&	
				$GLOBALS['TYPO3_DB']->sql_select_db(TYPO3_db)) {
				$TTA[]= "<br/>eID eidtools after pconnect Elapsed Time :".(microtime(true)-$GLOBALS['g_TT']).' s';	
				$dbConnected = true;
			}
		}
		return $dbConnected;
