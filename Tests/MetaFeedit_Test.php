...
class Tx_MetaFeedit_Tests_MetaFeedit_Test extends tx_phpunit_testcase {
	...
	public function getMetaFeeditVar() {	
		...
		//$_POST['test[id]']=4;
		$res=$this->metafeeditlib->getMetaFeeditVar($conf,'test');
		self::assertTrue ($res==4, 'Couldnt get test value;'); 
	}
	/**
	 * @test
	 */
	public function filterFileNameCharacters() {
		
		//$_POST['test[id]']=4;
		$res=$this->metafeeditlib->filterFileNameCharacters('?*.|');
		self::assertTrue ($res=='____', 'File name filter ko :'.$res);
		$res=$this->metafeeditlib->filterFileNameCharacters('<">a');
		self::assertTrue ($res=='___a', 'File name filter ko :'.$res);
		$res=$this->metafeeditlib->filterFileNameCharacters("*'?b");
		self::assertTrue ($res=='___b', 'File name filter ko :'.$res);
	}
}
